...
function stringifyArgs(args?: {[key: string]: any}) {
  if (!args) {
    return '';
  }
  const argArr: string[] = [];
...
function makeUsersQuery(args?: {[key: string]: any}) {
...
test('should handle pagination at the root', async () => {
  ...
  const {data, errors} = await graphql(schemaRelay, query);
...
test('should reject an invalid cursor', async () => {
  ...
  const {errors} = await graphql(schemaRelay, query);
...
function makePostsQuery(args?: {[key: string]: any}) {
  let argString = stringifyArgs(args);
  return `{
    user(id: 1) {
      posts${argString} {
...
test('should handle pagination in a nested field', async () => {
  ...
  expect(posts.edges).toHaveLength(8);
...
test('nested paging should handle "last" and "before" args', async () => {
  ...
  expect(data.user.posts.pageInfo).toEqual(expected);
...
test('can handle nested pagination', async () => {
  ...
  expect(data.users.edges[0].node.posts.edges).toHaveLength(2);
...
test('can handle deeply nested pagination', async () => {
  ...
  expect(comments.pageInfo).toEqual(expected);
  expect(comments.edges).toHaveLength(3);
